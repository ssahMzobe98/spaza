...
if(isset($_SESSION['user_agent'],$_SESSION['var_agent'])){
  ...
  if($cur_user_row['user_type']==Constants::USER_TYPE_APP){
    if(isset($_GET['min'],$_GET['limit'])){
      ...
      $ordersCount = $orderPdo->getHistoryOrdersOfThisUserCount($cur_user_row['id']);
      $orders = $orderPdo->getHistoryOrdersOfThisUser($cur_user_row['id'],$_GET['min'],$_GET['limit']);
      $trig = count($orders);
      $min=($_GET['min']-10<0)?0:$_GET['min']-10;
      $limit=($_GET['limit']-10<10)?10:$_GET['limit']-10;
      $minLevel=$_GET['min']+10;
      $limitLevel=$_GET['limit']+10;
      $nav = "loadAfterQuery('.displayOrderData','../model/orderHistory.php?min={$minLevel}&limit={$limitLevel}')";
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            foreach($orders as $order){
              ?>
                <tr>
                    <td onclick="getOrderHistoryInfo(<?php echo $order['order_id'];?>)" style="color:#000000;">#<?php echo $order['order_id'];?></td>
            ...
            <tfoot>
              <tr>
                <th><div class='button'>
                      <a onclick="loadAfterQuery('.displayOrderData','../model/orderHistory.php?min=<?php echo $min;?>&limit=<?php echo $limit;?>');">prev</a>
                    </div>
                </th>
                <th></th>
                <th></th>
