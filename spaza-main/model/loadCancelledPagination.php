...
if(isset($_SESSION['user_agent'],$_SESSION['var_agent'])){
  ...
  if($cur_user_row['user_type']==Constants::USER_TYPE_ADMIN){
    if(isset($_GET['min'],$_GET['limit'])){
      $ordersCount = $orderPdo->getAllStatusCount(14,15);
      $orders = $orderPdo->getAllStatusOrder(14,15,$_GET['min'],$_GET['limit']);
      $trig = count($orders);
      $min=($_GET['min']-10<0)?0:$_GET['min']-10;
      $limit=($_GET['limit']-10<10)?10:$_GET['limit']-10;
      $minLevel=$_GET['min']+10;
      $limitLevel=$_GET['limit']+10;
      $nav = "loadAfterQuery('.displayOrderData','../model/loadCancelledPagination.php?min={$minLevel}&limit={$limitLevel}')";
      ...
      <table class="table table-striped">
        ...
        <tbody>
          ...
          <tfoot>
            <tr>
              <th><div class='button'>
                    <a onclick="loadAfterQuery('.displayOrderData','../model/loadCancelledPagination.php?min=<?php echo $min;?>&limit=<?php echo $limit;?>');">prev</a>
                  </div>
              </th>
              <th></th>
              <th></th>
